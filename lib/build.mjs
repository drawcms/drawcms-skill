...
/**
 * Convert one document node back into an authoring-spec node.
 * Document nodes carry presentation in `data` (`{ label, type, fillColor, … }`)
 * and a discriminator `type: "customShape"`; the input schema instead wants
 * `label`/`type` flat and rejects everything else, so only the authored fields
 * survive. `position` is kept deliberately — it is how a layout someone dragged
 * in the editor survives a pull → edit → push round trip.
 */
function nodeToSpec(node) {
  if (!node || typeof node !== "object") return node;
  // Already spec-shaped (no presentation envelope): pass through untouched.
  if (!node.data || typeof node.data !== "object") return node;
  const spec = {};
  if (node.id !== undefined) spec.id = node.id;
  if (node.data.label !== undefined) spec.label = node.data.label;
  if (node.data.type !== undefined) spec.type = node.data.type;
  if (node.position && typeof node.position === "object") spec.position = node.position;
  const motion = motionToSpec(node.data);
  if (motion) spec.motion = motion;
  return spec;
}

/**
 * Convert one document edge back into an authoring-spec edge. Routing
 * (`sourceHandle`, `markerEnd`, `data.diagramRoute`, …) is engine-derived and
 * regenerated on the next build, so it is dropped. The sequence message kind
 * lives at `data.sequenceType` in a document and at `type` in a spec, using the
 * same vocabulary.
 */
function edgeToSpec(edge) {
  if (!edge || typeof edge !== "object") return edge;
  if (!edge.data || typeof edge.data !== "object") return edge;
  const spec = {};
  // Keep the id: beats/story target edges by id, so dropping it would orphan
  // every edge reference in the story we are about to carry over.
  if (edge.id !== undefined) spec.id = edge.id;
  if (edge.source !== undefined) spec.source = edge.source;
  if (edge.target !== undefined) spec.target = edge.target;
  const label = edge.data.label ?? edge.label;
  if (label !== undefined) spec.label = label;
  if (edge.data.sequenceType !== undefined) spec.type = edge.data.sequenceType;
  const motion = motionToSpec(edge.data);
  if (motion) spec.motion = motion;
  return spec;
}

/** `data.{preset,motionLoop,motionSpeed}` → spec `motion.{preset,loop,speed}`. */
function motionToSpec(data) {
  if (!data.preset) return undefined;
  const motion = { preset: data.preset };
  if (typeof data.motionLoop === "boolean") motion.loop = data.motionLoop;
  if (typeof data.motionSpeed === "number") motion.speed = data.motionSpeed;
  return motion;
}
 ...
 * ({ diagramType, nodes, edges, beats? }) or a full built/pulled document
 * ({ schemaVersion, meta, canvas, nodes, edges, assets, motion }).
 *
 * This has to be a real inverse of the build, not just a `meta` strip: the input
 * schema is strict, and a document's nodes/edges are wrapped in presentation
 * (`data.label`, `type: "customShape"`, routing) that the schema rejects. Before
 * this existed, `push` wrote a document to `.drawcms/<name>.json` that the CLI
 * could not read back, so a second `push` of the same diagram — and the whole
 * documented `pull` → `edit` → `push` loop — failed with
 * `invalid_type nodes.0.label Required`.
 *
 * Engine-derived detail (canvas, assets, schemaVersion, edge routing, node
 * colors and sizing) is dropped because the next build regenerates it.
 * Deliberately preserved: node `position`, so a human-arranged layout survives,
 * and the compiled `motion.story`, so the walkthrough is not lost.
 *
 * One known consequence: element motion in a document is indistinguishable from
 * motion the author asked for, so a round-tripped spec presents all of it as
 * authored. A later `--static` build therefore keeps it instead of stripping it.
 * Preserving what the document actually contains is the right default for a sync
 * tool; author from the original spec when you want static output.
 *
 * Authoring specs (no `meta`) pass through unchanged.
 */
export function toBuildSpec(doc) {
  ...
  // Top level: only the six keys the strict input schema accepts.
  if (doc.diagramType !== undefined) spec.diagramType = doc.diagramType;
  else if (meta && typeof meta === "object" && meta.diagramType) spec.diagramType = meta.diagramType;
  if (doc.name !== undefined) spec.name = doc.name;
  else if (meta && typeof meta === "object" && meta.name) spec.name = meta.name;

  if (Array.isArray(doc.nodes)) spec.nodes = doc.nodes.map(nodeToSpec);
  if (Array.isArray(doc.edges)) spec.edges = doc.edges.map(edgeToSpec);

  // `beats` is authoring sugar the engine compiles away, so a document never
  // has it — but a hand-edited file might carry both. Keep it if present, and
  // otherwise recover the walkthrough from the compiled story.
  if (doc.beats !== undefined) spec.beats = doc.beats;
  if (doc.story !== undefined) spec.story = doc.story;
  else if (doc.motion && typeof doc.motion === "object" && doc.motion.story) {
    spec.story = doc.motion.story;
  }
