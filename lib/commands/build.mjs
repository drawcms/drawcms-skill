...
const HELP = `drawcms build <type> <spec.json> [--json] [--out <file>] [--animate]
...
STATIC — engine-derived element motion is stripped so the story is a clickable
walkthrough without animation. Pass --animate to keep the derived motion, or set
motion explicitly on a node/edge in the spec (that is always kept).
  --out <file>   Write the built document JSON to <file> (default: stdout summary only).
  --animate      Keep engine-derived motion presets (default: static).
...
export async function run({ flags, positional }) {
  ...
  const typeResult = resolveDiagramType(cliType, read.spec);
  if (!typeResult.ok) {
  ...
  const result = buildDocument(spec, { animate: flags.animate === true });
  if (!result.ok) {
